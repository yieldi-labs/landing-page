...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 w-full z-40">
      <nav
        className={twJoin(
          "w-full md:static md:text-sm flex",
          state && "fixed z-10 h-full",
          scrolled && "md:bg-[#F5F1EB] bg-transparent",
        )}
      >
        <div className="md:custom-screen md:mx-auto gap-2 md:py-5 md:flex flex w-full md:w-auto items-end md:items-center flex-col md:flex-row lg:w-[990px]">
          <div
            className={`custom-screen items-center mx-auto p-4 md:p-0 flex justify-between w-full ${scrolled ? "bg-[#F5F1EB] md:bg-transparent" : ""}`}
          >
            <Brand />
          ...
          <div
            className={`flex-1 pb-3 md:pb-0 md:flex ${state ? "" : "hidden"} justify-between align-middle`}
          >
            <ul
              className="text-[#003944] md:flex md:space-x-8 mx-5 md:space-y-0 md:font-medium py-2 md:py-0
                ...
                return (
                  <li key={idx}>
                    <Link
                      href={item.path}
                      key={idx}
                      className="duration-150 hover:text-[#F5F1EB] hover:bg-[#332b29c9] h-10 px-5 py-2.5 md:rounded-[40px] md:border md:border-[#514a47] 
